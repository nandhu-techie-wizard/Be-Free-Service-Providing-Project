...
export function Adminvender(){
    var {userid}=useParams()
    const[details,setDetails]=useState([])
    useEffect(()=>{
      fetch("http://localhost:3005/getvender")
      .then((res) => res.json())
  ...
  function deleted(userid){
    ...
    return(
      ...
      <h1 className="text-success text-center">All venders Details.</h1>
       ...
       <div className="card-body">
           ...
           <h5 className="p-2"> {value.location }</h5><br/>
         ...
         <Link to={`/vender/${value.userid}`}><button className="btn btn-info ms-4" >Veiw Page</button></Link>
